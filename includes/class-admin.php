	...
	public static function card( $job, $station ) {
		...
		<article class="psf-card<?php echo $held ? ' is-held' : ''; ?>">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" class="psf-moves">
				...
				<?php if ( $held ) : ?>
					<button name="psf_action" value="resume"><?php echo esc_html__( 'Resume', 'public-shop-floor' ); ?></button>
				<?php else : ?>
					<?php if ( PSF_Stations::prev_key( $job['station_key'] ) ) : ?>
						<button name="psf_action" value="back"><?php echo esc_html__( 'Send back', 'public-shop-floor' ); ?></button>
					<?php endif; ?>
					<button name="psf_action" value="hold"><?php echo esc_html__( 'Hold', 'public-shop-floor' ); ?></button>
					<?php if ( PSF_Stations::next_key( $job['station_key'] ) ) : ?>
						<button name="psf_action" value="advance" class="primary"><?php echo esc_html__( 'Advance to next', 'public-shop-floor' ); ?></button>
					<?php else : ?>
						<button name="psf_action" value="complete" class="primary"><?php echo esc_html__( 'Leave the floor', 'public-shop-floor' ); ?></button>
					<?php endif; ?>
				<?php endif; ?>
			</form>
		</article>
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					...
					<tr>
						...
						<td>
							<label><input type="checkbox" name="<?php echo esc_attr( PSF_Plugin::OPTION ); ?>[public_board]" value="1" <?php checked( $s['public_board'], '1' ); ?> /> <?php echo esc_html__( 'Anyone can see jobs on /shop-floor/ (no customer names)', 'public-shop-floor' ); ?></label>
						</td>
					</tr>
					<tr>
						<th><?php echo esc_html__( 'Product names', 'public-shop-floor' ); ?></th>
						<td>
							<label><input type="checkbox" name="<?php echo esc_attr( PSF_Plugin::OPTION ); ?>[show_product]" value="1" <?php checked( $s['show_product'], '1' ); ?> /> <?php echo esc_html__( 'Show product titles on the public board', 'public-shop-floor' ); ?></label>
						</td>
					</tr>
					<tr>
